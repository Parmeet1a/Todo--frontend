...
const Task = () => {
  ...
  const [ownerInput,setOwnwrInput] = useState(false);
  ...
  const handleUpdate = (_id) => {
    ...
    const taskSearch = (e) =>{
     ...
     const filter = a.favArray.filter((value)=>{return (value.title.toLowerCase()).includes(inputValue)})
     
    //  console.log(filter);
     
        a.setFavArray(filter);
        ...
        >
          <div className=" flex items-center ml-6 ">
            <div className="text-xl font-bold">
              Tasks(<span className="font-semibold indent-4">1</span>)
            </div>
          ...
          {a.favArray.map((val) => (
            <>
              ...
              >
                ...
                <div className=" w-full h-full flex items-center  justify-center font-serif">
                 {ownerInput=== true && helpId === val._id ? <input className="outline-none" type="text" onChange={(e)=>{if(helpId === val._id){setOwner(e.target.value)}}} onKeyDown={(e)=>{ if (e.key === "Enter"){setOwner(owner);setOwnwrInput(false)}}} value={owner} />
                  : <div onClick={()=>{setOwnwrInput(true);setHelpId(val._id)}} className=" w-full h-full cursor-pointer flex items-center justify-center">{owner || val.owner}</div>}
                </div>
              </div>
